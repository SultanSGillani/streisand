...
class Film(models.Model):
    ...
    genre_tags = models.ManyToManyField(
        'media_formats.Genre', related_name='films', blank=True)
    ...
    def genre_tags_indexing(self):
        ...
        return json.dumps(
            [genre_tag.name for genre_tag in self.genre_tags.all()])

    def fetch_rotten_tomatoes_info(self):
        if self.rotten_tomatoes:
            self.rotten_tomatoes.refresh_from_api()
        elif self.imdb:
            self.rotten_tomatoes = FilmRottenTomatoes.objects.create_from_imdb_tt_id(
                self.imdb.tt_id)
            self.save()
...
class Collection(models.Model):
    creator = models.ForeignKey(
        ...
        null=True,
        related_name='collection_creators')
