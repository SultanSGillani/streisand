import * as React from 'react';
import { connect } from 'react-redux';

import Store from '../../state/store';
import { parsePageNumber } from '../../utilities/shim';
import Loading from '../../components/generic/Loading';
import CollectionsView from '../../components/Collections/CollectionsView';
import { IDispatch } from '../../state/actions/ActionTypes';
import { getCollections } from '../../state/Collection/actions/CollectionsAction';
import ILoadingStatus, { defaultStatus } from '../../models/base/ILoadingStatus';

export type Props = {
    params: {
        page: string;
    };
};

type ConnectedState = {
    page: number;
    status: ILoadingStatus;
};

type ConnectedDispatch = {
    getCollections: (page: number) => void;
};

type CombinedProps = ConnectedState & ConnectedDispatch & Props;
class CollectionsPage extends React.Component<CombinedProps> {
    public componentWillMount() {
        if (!this.props.status.loading) {
            this.props.getCollections(this.props.page);
        }
    }

    public componentWillReceiveProps(props: CombinedProps) {
        const status = props.status;
        const needPage = !status.failed && (!status.loaded || status.outdated);
        const pageChanged = props.page !== this.props.page;
        if (!status.loading && (pageChanged || needPage)) {
            this.props.getCollections(props.page);
        }
    }

    public render() {
        if (!this.props.status.loaded) {
            return <Loading />;
        }

        return (
            <CollectionsView page={this.props.page} />
        );
    }
}

const mapStateToProps = (state: Store.All, props: Props): ConnectedState => {
    const pageNumber = parsePageNumber(props.params && props.params.page);
    const page = state.sealed.collection.list.pages[pageNumber];
    return {
        page: pageNumber,
        status: page ? page.status : defaultStatus
    };
};

const mapDispatchToProps = (dispatch: IDispatch): ConnectedDispatch => ({
    getCollections: (page: number) => dispatch(getCollections(page))
});

export default connect(mapStateToProps, mapDispatchToProps)(CollectionsPage);
